...
export const DisplayMapComponent = () =>
{
	...
	React.useEffect(() =>
	{
		...
		{
			const newMarker = new H.map.Marker({ lat: parseFloat(restaurant.latitude), lng: parseFloat(restaurant.longitude) })
			newMarker.setData(`<div><a href="${restaurant.website}">${restaurant.name}</a><br>`)
			if(restaurantIcon) {
				newMarker.setIcon(restaurantIcon)
			}
			newMarker.addEventListener('tap', tapevent=>{
				const bubble = new H.ui.InfoBubble({lat:parseFloat(restaurant.latitude), lng: parseFloat(restaurant.longitude)},
				 {
				  content: tapevent.target.getData()
				})
				ui.addBubble(bubble);
